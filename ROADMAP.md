...
- [ ] **Phase 2 — src-Konsolidierung + wgnd-Toolkit** — ein Paket `us_used_vehicle_resales/`, Namenskollision `wgnd` auflösen, echtes Toolkit als Git-Dependency, `pyproject.toml` neu, Notebook-Imports repointen.
- [ ] **Phase 3 — Notebook-Hygiene** — Intro bereinigen (LLM-Text raus), Modell-Notebooks als `03a/03b` + `04_evaluation`, fehlende `show_df()`-Tabellen, lose Artefakte verschieben.
