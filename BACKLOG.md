...
| ~~11~~ | ✅ Phase 5: `/project-review` (2026-07-16) + `/project-case`-Artefakte + manuelle Durchsicht (#21) abgeschlossen (2026-07-17). | 1 | project-review 2026-06-27 |
...
| 15 | **Notebooks reproduzierbar durchlaufen** — Stand 2026-07-17: `01`/`01a`/`01b`/`02`/`03_modelling-prep`/`04`/`04a`/`04b`/`05` frisch durchgelaufen. **Weiterhin offen: `03a`, `03b`, `06`, `07`** (Outputs noch aus der ursprünglichen Lernphase). Entscheidung 2026-07-09: Outputs bewusst committet behalten, `nbstripout`-Dep entfernt (war inaktiv). | 2 | content-audit 2026-07-09 |
...
| ~~21~~ | ✅ **Manuelle Durchsicht der public-Artefakte** abgeschlossen (2026-07-17, ~6 Feedback-Runden) — Styleguide-Sprung, Agenda-Restrukturierung, vereinheitlichte Closing-Slide, TechView-Robustheits-Slide, zahlreicher Content-Feinschliff. Details → PROCESS_LOG. | 1 | Kay 2026-07-09 |
| ~~24~~ | ✅ **Entscheidung getroffen (2026-07-17):** "448 Runs" bleibt als historischer Snapshot stehen (Stand des ursprünglichen systematischen Sweeps vom 9.7.). Ursache für den Sprung auf real 562 Zeilen identifiziert: `03a_modelling-logreg.ipynb` + `03b_modelling-rf.ipynb` schreiben über `tracker.add_entry()` (8 Aufrufstellen, teils in Schleifen — Grid-Search, RF-Experimente) in dieselbe persistente `model_results_tracking.csv`; beide wurden zuletzt bei der Boilerplate-Konsolidierung (BACKLOG #22, 16.7.) ausgeführt. Nicht durch die Notebook-Reruns vom 17.7. verursacht — CSV-Zeitstempel (12:35 Uhr) liegt vor dem Großteil der heutigen Session, seither unverändert. Bewusst nicht rückwirkend in README/Story nachgezogen (Aufwand: ~15 Stellen + Pipeline-Regenerierung + Preview-Verifikation). Falls je aktualisiert: "448" konsequent durch den dann aktuellen Wert ersetzen, nicht mischen. | 3 | Kay 2026-07-17 |
| 25 | **GitHub Pages-Aktivierung bestätigen** — `.github/workflows/pages.yml` + `public/.nojekyll` sind angelegt, aber Settings → Pages → Source: GitHub Actions muss noch im GitHub-Web-UI gesetzt werden (kein `gh`-CLI lokal verfügbar). | 1 | Kay 2026-07-17 |
